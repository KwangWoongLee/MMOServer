...
void Lock::WriteLock(const char* name)
{
	...
	const uint32_t lockThreadId = (mLockFlag.load() & WRITE_THREAD_MASK) >> 16;
	...
	const uint32_t desired = ((TLS_ThreadId << 16) & WRITE_THREAD_MASK);
	while (true)
	{
...
void Lock::WriteUnlock(const char* name)
{
	...
	const int32_t lockCount = --mWriteCount;
	if (lockCount == 0)
...
void Lock::ReadLock(const char* name)
{
	...
	while (true)
	{
		for (uint32_t spinCount = 0; spinCount < MAX_SPIN_COUNT; spinCount++)
		{
			uint32_t expected = (mLockFlag.load() & READ_COUNT_MASK);
